...
function StatCard({ icon: Icon, label, value, color = "#a855f7" }: any) {
  return (
    <div
      className="rounded-2xl p-5 border border-white/10 bg-[#1a1528]"
...
function PlanBadge({ plan }: { plan: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-2.5 py-0.5 rounded-lg text-[11px] font-black uppercase tracking-wider border ${styles[plan] || styles.free}`}
    >
      {plan}
    </span>
  );
...
export default function Admin() {
  ...
  const loadExtMeta = async () => {
    try {
      const key = localStorage.getItem("admin_key");
      const res = await fetch(`${API_BASE}/api/admin/extension-meta`, {
  ...
  const handleExtUpload = async (file: File) => {
    if (!file.name.endsWith(".zip")) {
      setExtMsg({ type: "err", text: "Only ZIP files allowed" });
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/admin/extension-upload`, {
        method: "POST",
        headers: key ? { "x-admin-key": key } : {},
        body: form,
      });
      ...
      setExtMsg({
        type: "ok",
        text: `✅ "${file.name}" uploaded — ${(file.size / 1024).toFixed(1)} KB`,
      });
    } catch (err: any) {
  ...
  const toggleSession = async (id: number, isActive: boolean) => {
    try {
      await adminApi.fetch(`/sessions/${id}`, {
        method: "PATCH",
    ...
    try {
      ...
      if (geminiEditId !== null) {
        await adminApi.fetch(`/gemini-sessions/${geminiEditId}`, {
          method: "PATCH",
          body: JSON.stringify({
            label: geminiSessionLabel,
            cookiesJson: geminiSessionCookies,
          }),
        });
      ...
      setGeminiSessionLabel("");
      setGeminiSessionCookies("");
      setGeminiEditId(null);
      loadData();
    } catch (err: any) {
      toast({
        title: "Failed",
  ...
  const toggleGeminiSession = async (id: number, isActive: boolean) => {
    try {
      await adminApi.fetch(`/gemini-sessions/${id}`, {
        method: "PATCH",
        body: JSON.stringify({ isActive }),
      });
      loadData();
    ...
    try {
      ...
      if (whiskEditId !== null) {
        await adminApi.fetch(`/whisk-sessions/${whiskEditId}`, {
          method: "PATCH",
          body: JSON.stringify({
            label: whiskSessionLabel,
            cookiesJson: whiskSessionCookies,
          }),
        });
      ...
      setWhiskSessionLabel("");
      setWhiskSessionCookies("");
      setWhiskEditId(null);
      loadData();
    } catch (err: any) {
      toast({
  ...
  const toggleWhiskSession = async (id: number, isActive: boolean) => {
    try {
      await adminApi.fetch(`/whisk-sessions/${id}`, {
        method: "PATCH",
        body: JSON.stringify({ isActive }),
      });
      loadData();
  ...
  const suspendUser = async (u: any) => {
    if (!confirm(`Suspend "${u.username}"? Their credits will be set to 0.`))
      return;
    try {
  ...
  const fixReferral = async (u: any) => {
    if (
      !confirm(
        `Fix referral reward for "${u.username}"? This will award tokens to their referrer if not already done.`,
      )
    )
      return;
    try {
      const data = await adminApi.fetch(`/users/${u.id}/fix-referral`, {
        method: "POST",
      });
      toast({
        title: "Referral fixed!",
        description: `${data.tokensAwarded} tokens awarded to ${data.referrer}`,
      });
      loadData();
    } catch (err: any) {
      toast({
    ...
    return (
      ...
      >
        ...
        <div className="relative w-full max-w-md z-10">
          <div
            className="rounded-2xl border-2 border-[#7c3aed] bg-[#12101a] p-8 space-y-7"
            style={{ boxShadow: "8px 8px 0px #4c1d95" }}
          >
            <div className="text-center space-y-3">
              <div
                className="h-14 w-14 rounded-2xl bg-[#7c3aed]/20 flex items-center justify-center mx-auto border border-[#7c3aed]/40"
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-white" style={FONT}>
      ...
      <header className="sticky top-0 z-40 h-14 bg-[#0d0b14] border-b border-white/8 flex items-center px-6 gap-5">
        <div className="flex items-center gap-2.5 shrink-0">
          ...
          <span className="font-black text-base text-white">
            Bunny<span className="text-[#a855f7]">Flow</span> Admin
          </span>
        </div>
      ...
      <main className="p-6 space-y-6">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <StatCard
                ...
                value={`${sessions.filter((s) => s.isActive).length} / ${sessions.length}`}
                color="#34d399"
              />
              <StatCard
                icon={CheckCircle2}
            ...
            {users.length > 0 && (
              <div
                className="rounded-2xl bg-[#1a1528] border border-white/8 overflow-hidden"
              ...
              >
                <div className="px-5 py-4 border-b border-white/8 flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 text-xs text-[#a855f7] font-bold hover:underline"
                  >
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-500 text-sm mt-0.5">
                  Manage cookie sessions for Google Flow
                </p>
              </div>
              <button
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-white/8">
                      {["Label", "Added", "Usage", "Status", "Actions"].map(
                        (h) => (
                          <th
                            key={h}
                            className="text-left px-5 py-3.5 text-[10px] font-black text-gray-500 uppercase tracking-widest"
                          >
                            {h}
                          </th>
                        ),
                  ...
                  <tbody className="divide-y divide-white/5">
                    {sessions.map((s) => (
                      ...
                      >
                        ...
                        <td className="px-5 py-3.5 text-gray-400 text-sm font-semibold">
                          {s.usageCount}
                        </td>
                        <td className="px-5 py-3.5">
                          ...
                          >
                            {s.isActive ? (
                              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg text-[11px] font-black bg-green-500/15 text-green-400 border border-green-500/25">
                                <CheckCircle2 className="h-3 w-3" />
                                Active
                              </span>
                            ) : (
                              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg text-[11px] font-black bg-white/8 text-gray-400 border border-white/10">
            ...
            {showAddSession && (
              <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
                <div
                  className="w-full max-w-lg rounded-2xl bg-[#12101a] border border-[#7c3aed]/40 p-6 space-y-5"
                  style={{ boxShadow: "0 0 40px rgba(124,58,237,0.2)" }}
                >
                  <div>
                    <h3 className="text-lg font-black text-white">
                      Add Google Flow Session
                    </h3>
                    <p className="text-gray-500 text-sm mt-0.5">
                      Paste exported cookie JSON from your browser
                    </p>
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-sm font-bold text-gray-300">
                  ...
                  <div className="flex gap-3">
                    <button
                      ...
                      className="flex-1 h-11 rounded-xl border border-white/15 text-gray-400 font-bold text-sm hover:bg-white/5 transition-colors"
                    >
                      Cancel
                    </button>
                    <button
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-500 text-sm mt-0.5">
                  Manage cookie sessions for Google Gemini Pro
                </p>
              </div>
              <button
                onClick={() => {
            ...
            >
              {geminiSessions.length === 0 ? (
                <div className="py-16 text-center">
                  ...
                  <p className="text-gray-500 font-semibold">
                    No Gemini sessions added yet
                  </p>
                  <p className="text-gray-600 text-xs mt-1">
                    Add a session to enable Gemini Pro access
                  </p>
                </div>
              ) : (
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-white/8">
                      {["Label", "Added", "Cookies", "Status", "Actions"].map(
                        (h) => (
                          <th
                            key={h}
                            className="text-left px-5 py-3.5 text-[10px] font-black text-gray-500 uppercase tracking-widest"
                    ...
                    {geminiSessions.map((s) => (
                      ...
                      >
                        ...
                        <td className="px-5 py-3.5">
                          <button
                            onClick={() =>
                              toggleGeminiSession(s.id, !s.isActive)
                            }
                          >
                            {s.isActive ? (
                              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg text-[11px] font-black bg-green-500/15 text-green-400 border border-green-500/25">
                        ...
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() => {
                                setGeminiEditId(s.id);
                                setGeminiSessionLabel(s.label);
                                setGeminiSessionCookies(s.cookiesJson || "");
                                setShowAddGeminiSession(true);
                              }}
                            ...
                            <button
                              onClick={() => deleteGeminiSession(s.id)}
                              className="h-8 w-8 rounded-lg bg-red-500/10 hover:bg-red-500/20 flex items-center justify-center text-red-400 transition-colors border border-red-500/20"
                            >
                              <Trash2 className="h-3.5 w-3.5" />
                            </button>
                ...
                >
                  <div>
                    <h3 className="text-lg font-black text-white">
                      {geminiEditId
                        ? "Edit Gemini Session"
                        : "Add Gemini Pro Session"}
                    </h3>
                    <p className="text-gray-500 text-sm mt-0.5">
                      {geminiEditId
                        ? "Update the cookie JSON for this session"
                        : "Paste exported cookie JSON from your Gemini browser session"}
                    </p>
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-sm font-bold text-gray-300">
                      Label (e.g. Gemini Account 1)
                    </label>
                    <input
                      value={geminiSessionLabel}
                      onChange={(e) => setGeminiSessionLabel(e.target.value)}
                      className="w-full h-11 rounded-xl bg-[#1a1528] border-2 border-white/10 focus:border-[#7c3aed] text-white px-4 text-sm outline-none transition-colors"
                  ...
                  <div className="space-y-1.5">
                    ...
                    <textarea
                      ...
                      onChange={(e) => setGeminiSessionCookies(e.target.value)}
                      rows={6}
                      className="w-full rounded-xl bg-[#1a1528] border-2 border-white/10 focus:border-[#7c3aed] text-white px-4 py-3 text-xs font-mono outline-none transition-colors resize-none"
                      placeholder='[{"name": "SID", "value": "..."}, ...]'
                    />
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-500 text-sm mt-0.5">
                  Manage cookie sessions for Google Whisk AI image generation
                </p>
              </div>
              <button
                onClick={() => {
            ...
            >
              {whiskSessions.length === 0 ? (
                <div className="py-16 text-center">
                  ...
                  <p className="text-gray-500 font-semibold">
                    No Whisk sessions added yet
                  </p>
                  <p className="text-gray-600 text-xs mt-1">
                    Add a session to enable Whisk AI access
                  </p>
                </div>
              ) : (
                <table className="w-full">
                    ...
                    {whiskSessions.map((s: any) => (
                      ...
                      >
                        ...
                        <td className="px-5 py-3.5">
                          <button
                            onClick={() =>
                              toggleWhiskSession(s.id, !s.isActive)
                            }
                          >
                            {s.isActive ? (
                              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg text-[11px] font-black bg-green-500/15 text-green-400 border border-green-500/25">
                        ...
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() => {
                                setWhiskEditId(s.id);
                                setWhiskSessionLabel(s.label);
                                setWhiskSessionCookies(s.cookiesJson || "");
                                setShowAddWhiskSession(true);
                              }}
                            ...
                            <button
                              onClick={() => deleteWhiskSession(s.id)}
                              className="h-8 w-8 rounded-lg bg-red-500/10 hover:bg-red-500/20 flex items-center justify-center text-red-400 transition-colors border border-red-500/20"
                            >
                              <Trash2 className="h-3.5 w-3.5" />
                            </button>
            ...
            {showAddWhiskSession && (
              <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
                <div
                  className="w-full max-w-lg rounded-2xl bg-[#12101a] border p-6 space-y-5"
                  style={{
                ...
                >
                  <div>
                    <h3 className="text-lg font-black text-white">
                      {whiskEditId ? "Edit Whisk Session" : "Add Whisk Session"}
                    </h3>
                    <p className="text-gray-500 text-sm mt-0.5">
                      {whiskEditId
                        ? "Update the cookie JSON for this session"
                        : "Paste exported cookie JSON from your Whisk browser session"}
                    </p>
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-sm font-bold text-gray-300">
                      Label (e.g. Whisk Account 1)
                    </label>
                    <input
                      value={whiskSessionLabel}
                      onChange={(e) => setWhiskSessionLabel(e.target.value)}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <textarea
                      value={whiskSessionCookies}
                      onChange={(e) => setWhiskSessionCookies(e.target.value)}
                      rows={6}
        ...
        {tab === "users" && (
          <div className="space-y-4">
            ...
            <div
              className="rounded-2xl bg-[#1a1528] border border-white/8 overflow-x-auto"
              style={{ boxShadow: "0 0 20px rgba(124,58,237,0.06)" }}
            >
              {users.length === 0 ? (
                <div className="py-16 text-center">
                ...
                <div className="py-16 text-center">
                  ...
                  <p className="text-gray-600 text-sm mt-1">
                    No match for "
                    <span className="text-[#a855f7]">{userSearch}</span>"
                  </p>
                  <button
                    onClick={() => setUserSearch("")}
                    className="mt-3 text-sm text-[#a855f7] font-bold hover:underline"
                  >
                    Clear search
                  </button>
                </div>
                ...
                <table className="w-full min-w-[900px]">
                  <thead>
                    <tr className="border-b border-white/8">
                      {[
                        ...
                        <th
                          key={h}
                          className="text-left px-5 py-3.5 text-[10px] font-black text-gray-500 uppercase tracking-widest whitespace-nowrap"
                        >
                          {h}
                        </th>
                      ))}
                  ...
                  <tbody className="divide-y divide-white/5">
                    {filteredUsers.map((u) => {
                      ...
                      const expDate = u.planExpiresAt
                        ? new Date(u.planExpiresAt)
                        : null;
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-5 py-4">
                            <div className="flex items-center gap-3">
                              <div
                                className={`h-9 w-9 rounded-full flex items-center justify-center font-black text-sm border shrink-0 ${isSuspended ? "bg-red-500/15 text-red-400 border-red-500/25" : "bg-[#7c3aed]/20 text-[#a855f7] border-[#7c3aed]/25"}`}
                              >
                                {(u.username || "U")[0].toUpperCase()}
                              </div>
                              <div>
                                <p className="text-sm font-black text-white flex items-center gap-1.5">
                                  {u.username}
                                  {isSuspended && (
                                    <span className="text-[10px] font-black text-red-400 bg-red-500/15 px-1.5 py-0.5 rounded border border-red-500/20">
                                      SUSPENDED
                                    </span>
                                  )}
                                </p>
                          ...
                          <td className="px-5 py-4 whitespace-nowrap">
                            {expDate ? (
                              <div>
                                <p
                                  className={`text-sm font-bold ${isExpired ? "text-red-400" : daysLeft !== null && daysLeft <= 3 ? "text-yellow-400" : "text-white"}`}
                                >
                                  {expDate.toLocaleDateString()}
                                </p>
                                <p
                                  className={`text-[11px] font-semibold ${isExpired ? "text-red-500" : "text-gray-500"}`}
                                >
                          ...
                          <td className="px-5 py-4">
                            ...
                            <div className="text-[11px] text-gray-500">
                              {u.videosCount ?? 0}v · {u.imagesCount ?? 0}i
                            </div>
                          </td>
                          {/* Actions */}
                          <td className="px-5 py-4">
                            <div className="flex items-center gap-2">
                              ...
                              {u.referredBy && (
                                <button
                                  onClick={() => fixReferral(u)}
                                  title="Fix Referral Reward"
                      ...
                      <div key={w.id} className="p-5 space-y-3">
                        ...
                        {w.status === "approved" && (
                          <button
                            ...
                            className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-green-500/15 text-green-400 text-xs font-bold border border-green-500/25 hover:bg-green-500/25 transition-colors"
                          >
                            <CheckCircle2 className="h-3.5 w-3.5" /> Mark as
                            Paid
                          </button>
                        )}
                      </div>
        ...
        {tab === "settings" && (
          <div className="space-y-4">
            <div>
              ...
              <p className="text-gray-500 text-sm mt-0.5">
                Global platform settings — click a value to edit, then press
            ...
            >
              {settings.length === 0 ? (
                <div className="py-16 text-center">
                  ...
                  <p className="text-gray-600 text-sm mt-1">
                    Settings will appear here once added from the database
                  </p>
                </div>
              ) : (
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {settings.map((s: any) => {
                      ...
                      return (
                        ...
                        >
                          {/* Key */}
                          <td className="px-5 py-4 font-mono text-sm text-[#a855f7] font-bold">
                            {s.key}
                          </td>
                          {/* Type badge */}
                          <td className="px-5 py-4">
                          ...
                          <td className="px-5 py-4">
                            {isBool ? (
                              ...
                              >
                                <span
                                  className={`inline-block h-4 w-4 rounded-full transition-all duration-200 font-black text-[8px] flex items-center justify-center ${s.value ? "translate-x-8 bg-white text-[#7c3aed]" : "translate-x-1 bg-gray-500 text-white"}`}
                                >
                                  {s.value ? "ON" : ""}
                                </span>
                                <span
                                  className={`absolute text-[9px] font-black tracking-wider transition-all ${s.value ? "left-2 text-white" : "right-2 text-gray-500"}`}
                                >
                                  {s.value ? "ON" : "OFF"}
                                </span>
                              </button>
        ...
        {tab === "extension" && (
          <div className="space-y-6">
            <div>
              ...
              <p className="text-gray-500 text-sm mt-0.5">
                Current extension available for all users to download from their
                Extension page
              </p>
            </div>
            ...
            {/* Extension info card */}
            <div
              className="rounded-2xl bg-[#0d0b14] border border-[#7c3aed]/25 p-6"
              style={{ boxShadow: "0 0 30px rgba(124,58,237,0.07)" }}
            >
              <div className="flex items-center gap-4 mb-5">
                <div
                  className="h-14 w-14 rounded-2xl bg-[#7c3aed]/15 border border-[#7c3aed]/30 flex items-center justify-center flex-shrink-0"
                  style={{ boxShadow: "0 0 20px rgba(124,58,237,0.2)" }}
                >
                  <Download className="h-7 w-7 text-[#a855f7]" />
                </div>
                <div>
                  <p className="text-[#a855f7] font-black text-xs uppercase tracking-widest mb-1">
                    Active Extension
                  </p>
                  ...
                  <p className="text-gray-500 text-xs mt-0.5">
                    {extMeta?.size
                      ? `${(extMeta.size / 1024).toFixed(1)} KB`
                      : "~88 KB"}{" "}
                    · Version {extMeta?.version || "v3.6.6"} · Updated{" "}
                    {extMeta?.uploadedAt
                      ? new Date(extMeta.uploadedAt).toLocaleDateString()
                      : "today"}
                  </p>
                </div>
              </div>
                ...
                {[
                  {
                    ...
                    desc: "User goes to their Extension page → clicks Download Extension",
                  },
                  {
                    n: 2,
                    title: "Enable Developer Mode",
                    desc: 'Chrome → chrome://extensions → toggle "Developer mode" ON (top-right)',
                  },
                  {
                    n: 3,
                    title: "Remove old extension",
                    desc: "If BunnyFlow is already installed → Remove it first to avoid conflicts",
                  },
                  {
                    n: 4,
                    title: "Drag & Drop ZIP",
                    desc: "Drag the downloaded ZIP file directly onto the chrome://extensions page",
                  },
                  {
                    n: 5,
                    title: "Enter Server URL",
                    desc: `Click extension icon → Server URL field → paste: ${typeof window !== "undefined" ? window.location.origin : "https://flowbybunny.replit.app"} → Save`,
                  },
                  {
                    n: 6,
                    title: "Sign In",
                    desc: "Enter BunnyFlow email + password in popup → Sign In",
                  },
                  {
                    n: 7,
                    title: "Visit the site",
                    desc: "Open Google Flow or Whisk — session auto-injects ✅",
                  },
                ].map((step) => (
                  <div key={step.n} className="flex items-start gap-4">
                    <div className="h-6 w-6 rounded-full bg-[#7c3aed] flex items-center justify-center text-white text-xs font-black flex-shrink-0 mt-0.5">
                      {step.n}
                    </div>
                    <div>
      ...
      {editUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div
            className="w-full max-w-md rounded-2xl bg-[#12101a] border border-[#7c3aed]/40 p-6 space-y-5"
            style={{ boxShadow: "0 0 40px rgba(124,58,237,0.2)" }}
          >
            <div>
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-gray-500">
                Override the plan expiry — set any date you want
              </p>
              <input
                type="date"
                value={editExpiry}
